...
float E = 2;
int APD_start = E*10000;//50000;//
int APD_end = E*50000;//150000;//
int APD_bin = 201;
...
TH1F *calibrated_sum_hist = new TH1F("calibrated sum","calibrated sum",201*E,4*APD_start,4*APD_end);
float mean_APDs;
float mean_sides;
float mean_corners;
float APD_weights[4] = {1,1,1,1};//{0.95,0.93,1.17,0.98};//{1,1,1,1};
...
void resolution(){
  ...
  //TFile *center_run = new TFile("/afs/cern.ch/work/a/ajofrehe/cern-summer-2016/H4Analysis/ntuples/analysis_subtracted_summed_4684.root");
  TFile *center_run = new TFile("/afs/cern.ch/work/a/ajofrehe/cern-summer-2016/H4Analysis/ntuples/analysis_subtracted_summed_4684.root");
  ...
  mtree->SetBranchAddress("WF_val17",WF_val17);
  ...
  side[0] = new TH1F("crystal2","crystal2",100,0,20000*E);
  side[1] = new TH1F("crystal6","crystal6",100,0,20000*E);
  side[2] = new TH1F("crystal11","crystal11",100,0,20000*E);
  side[3] = new TH1F("crystal15","crystal15",100,0,20000*E);
  corner[0] = new TH1F("crystal1","crystal1",100,0,20000*E);
  corner[1] = new TH1F("crystal3","crystal1",100,0,20000*E);
  corner[2] = new TH1F("crystal14","crystal14",100,0,20000*E);
  corner[3] = new TH1F("crystal16","crystal16",100,0,20000*E);
  ...
  crys->SetParameters(500,weighted_sum_hist->GetMean(),(weighted_sum_hist->GetStdDev())/4,1.1,2.1);
  //crys->FixParameter(3,1.1);
  //crys->FixParameter(4,2.1);
  weighted_sum_hist->Draw();
  weighted_sum_hist->Fit("crys","","",70000*E,130000*E);//250000,450000);//
  ...
  crys->ReleaseParameter(3);
  crys->ReleaseParameter(4);
  crys->SetParameters(500,calibrated_sum_hist->GetMean(),(calibrated_sum_hist->GetStdDev())/4,1.1,2.1);
  //crys->FixParameter(0,430);
  //crys->FixParameter(3,1.1);
  //crys->FixParameter(4,2.5);
  calibrated_sum_hist->Draw();
  calibrated_sum_hist->Fit("crys","same","",70000*E,200000*E);//250000,550000);//

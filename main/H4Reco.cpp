...
int main(int argc, char* argv[])
{
    ...
    int eventStart=opts.GetOpt<int>("h4reco.EventStartNumber");
    ...
    TFile* outROOT = new TFile("ntuples/"+outSuffix+TString(run)+"s"+to_string(int(eventStart/maxEvents))+".root", "RECREATE");
    outROOT->cd();
    ...
    cout << "total number of events:    " << nentries << endl << endl << endl;
    if (maxEvents == -1) maxEvents = nentries - eventStart;
    //while(h4Tree.NextEntry() && (index-stoul(run)*1e9<maxEvents || maxEvents==-1))
    for (index=stoul(run)*1e9; index < stoul(run)*1e9 + eventStart + maxEvents;index++)
    {
        if(index % 500 == 0 && index-stoul(run)*1e9 >= eventStart)
        {
            if (maxEvents > nentries - eventStart) cout << ">>>Processed events: " << index-stoul(run)*1e9-eventStart << "/"
                 << nentries - eventStart//(maxEvents<0 ? h4Tree.GetEntries() : min((int)h4Tree.GetEntries(), maxEvents))
                 << endl;
            else cout << ">>>Processed events: " << index-stoul(run)*1e9-eventStart << "/"
                 << maxEvents//(maxEvents<0 ? h4Tree.GetEntries() : min((int)h4Tree.GetEntries(), maxEvents))
                 << endl;
        ...
        important = true;
        if (index-stoul(run)*1e9 < eventStart) important = false;
        for(auto& plugin : pluginSequence)
            important = plugin->ProcessEvent(h4Tree, pluginMap, opts, important);
        ...
        if (h4Tree.NextEntry() == false) break;
        //++index;
    }

...
//float xtable[] = {17,21.2,12.7,21.2,12.7};
//float ytable[] = {0,-4.3,-4.3,4.3,4.3};
float xtable[] = {0,8,-8,0,0};
float ytable[] = {0,0,0,8,-8};
...
bool HodoReco::ProcessEvent(const H4Tree& h4Tree, map<string, PluginBase*>& plugins, CfgManager& opts ,bool important)
{
	...
	offset=opts.GetOpt<float>("H4Hodo.hodoCorrection.hodoAlignOffset",i);
	  ...
	  hodoTree_.x[i/nPlanes_] = value + offset + xtable[table_position];
	  hodoTree_.nFibresOnX[i/nPlanes_] = fibersOn[i].size();
	}else{
	  hodoTree_.Y[i/nPlanes_] = value + offset;
	  hodoTree_.y[i/nPlanes_] = value + offset + ytable[table_position];
    ...
    if (TMath::Abs(hodoTree_.X[1]) < 10 && TMath::Abs(hodoTree_.Y[1]) < 10 && hodoTree_.nFibresOnX[1] < 3 && hodoTree_.nFibresOnY[1] < 3) return true;
    else return false;
    //return true;
}
